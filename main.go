...
import (
	"fmt"
	"github.com/fyne-io/examples/gui"
	guitheme "github.com/fyne-io/examples/gui/theme"
	"log"
	"net/url"
...
func main() {
	...
	a.Settings().SetTheme(&guitheme.CustomTheme{})
	a.SetIcon(theme.FyneLogo())
	logLifecycle(a)
	w := a.NewWindow("notion-wb")
	topWindow = w
	...
	setTutorial := func(t gui.Info) {
		if fyne.CurrentDevice().IsMobile() {
			child := a.NewWindow(t.Title)
			topWindow = child
			child.SetContent(t.View(topWindow, content))
			child.Show()
		...
		content.Objects = []fyne.CanvasObject{t.View(w, content)}
		content.Refresh()
	}
...
func makeNav(setTutorial func(tutorial gui.Info), loadPrevious bool) fyne.CanvasObject {
	a := fyne.CurrentApp()
